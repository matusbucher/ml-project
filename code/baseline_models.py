...
class BaselineSolutionLengthModel:
    def __init__(self, normalized_data: NormalizedData):
        self.normalized_data = normalized_data
        self.X_train = np.array([len(f.solution) for f in normalized_data.train_data]).reshape(-1, 1)
        self.X_test = np.array([len(f.solution) for f in normalized_data.test_data]).reshape(-1, 1)
        self.y_train = np.array(normalized_data.train_labels)
        self.y_test = np.array(normalized_data.test_labels)
    ...
    def predict(self, features: Features) -> float:
        return self.reg.predict(np.array([[len(features.solution)]]))[0]
